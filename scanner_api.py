...
def _filter_safe_targets(targets: list[str]) -> tuple[list[str], int]:
    """Normalize targets and drop anything blocked by SSRF protection."""
    normalized = [normalize_host(target) for target in targets if normalize_host(target)]
    if _ALLOW_INTERNAL_SCAN:
        return normalized, 0

    safe_targets = [target for target in normalized if _validate_target_ssrf(target)]
    blocked = len(normalized) - len(safe_targets)
    return safe_targets, blocked
...
async def run_scan(req: ScanRequest) -> dict:
    normalized_targets, blocked = _filter_safe_targets(req.targets)
    if blocked > 0 and not normalized_targets:
        return {
    ...
    if req.enumerate_subdomains:
        for host in sorted(normalized_targets):
            ...
            for sub in subs:
                normalized_sub = normalize_host(sub)
                if not normalized_sub:
                    continue
                if not _ALLOW_INTERNAL_SCAN and not _validate_target_ssrf(normalized_sub):
                    continue
                enumerated_targets.add(normalized_sub)
...
async def run_scan_stream(req: ScanRequest):
    ...
    normalized_targets, blocked = _filter_safe_targets(req.targets)
    if blocked > 0:
        yield log("WARN", f"Blocked {blocked} target(s) due to SSRF protection.")
    if blocked > 0 and not normalized_targets:
        yield emit("error", {
            "message": f"All {blocked} target(s) were blocked by SSRF protection. Set ALLOW_INTERNAL_SCAN=true to override."
        })
        yield emit("done", {
            "total_scanned": 0,
            "ok": 0,
        ...
        for host in sorted(normalized_targets):
            ...
            if subs:
                ...
                for sub in subs:
                    ...
                    if not sub:
                        continue
                    if not _ALLOW_INTERNAL_SCAN and not _validate_target_ssrf(sub):
                        blocked_subdomains += 1
                        continue
                ...
                if blocked_subdomains:
                    yield log("WARN", f"  Skipped {blocked_subdomains} enumerated subdomain(s) due to SSRF protection")
            else:
