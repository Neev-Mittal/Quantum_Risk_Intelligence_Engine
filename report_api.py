...
from src.dataset_service import get_enriched_cbom_dataset, get_subdomains_dataset
...
def _load_json(filename: str):
    ...
    try:
        ...
        if normalized == "enriched_cbom.json":
            return get_enriched_cbom_dataset(session)
        if normalized == "subdomains.json":
            return get_subdomains_dataset(session)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=int(os.getenv("REPORT_API_PORT", "8002")))
